...
func startControllers(
	...
	supervisorDeployment *v1.Deployment,
	kubeClient kubernetes.Interface,
	pinnipedClient versioned.Interface,
	kubeInformers informers.SharedInformerFactory,
	pinnipedInformers externalversions.SharedInformerFactory,
) {
	...
	controllerManager := controllerlib.NewManager().
		...
		WithController(
			generator.NewFederationDomainSecretsController(
				...
				func(fd *v1alpha1.FederationDomainStatus) *v12.LocalObjectReference {
...
func run(podInfo *downward.PodInfo, cfg *supervisor.Config) error {
	...
	kubeInformers := informers.NewSharedInformerFactoryWithOptions(
		...
		informers.WithNamespace(serverInstallationNamespace),
	)

	pinnipedInformers := externalversions.NewSharedInformerFactoryWithOptions(
		client.PinnipedSupervisor,
		defaultResyncInterval,
		externalversions.WithNamespace(serverInstallationNamespace),
	)
